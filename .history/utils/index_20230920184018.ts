...
// 向指定号码发送指定验证码
export const sendCode = (phone: string, code: number, callback: Function) => {
  const { ACCOUNT_SID, AUTH_TOKEN, Rest_URL, AppID } = process.env;

  console.log(ACCOUNT_SID, AUTH_TOKEN, Rest_URL, AppID);
  if (ACCOUNT_SID && AUTH_TOKEN && Rest_URL && AppID) {
    //1. 准备请求url
    /**
